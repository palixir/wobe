...
import type { Wobe, MaybePromise, WobePlugin, WobeResponse, Context } from 'wobe'
...
export const WobeGraphqlApolloPlugin = async ({
	...
	const introspection =
		options.introspection ?? (allowIntrospection === true ? true : !isProduction)
	...
	return (wobe: Wobe<unknown>) => {
		const getResponse = async (context: Context) => {
			const fetchEndpoint = async (request: Request) => {
				...
				const res = await server.executeHTTPGraphQLRequest({
					httpGraphQLRequest: {
						...
						body: request.method === 'GET' ? request.body : requestBody,
						// @ts-expect-error
					...
					context: async () => {
						...
						if (typeof apolloContext === 'function') {
							const apolloContextResult = await apolloContext(context)

							if (apolloContextResult && typeof apolloContextResult === 'object')
			...
			if (!graphqlMiddleware) return resolveWithTimeout(resolve, timeoutMs)
